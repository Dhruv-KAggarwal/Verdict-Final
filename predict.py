...
if __name__ == '__main__': 
    ...
    if len(train_folders) > 1:
        # Sort the folders by creation time and use the latest one
        train_folders.sort(key=lambda x: os.path.getctime(detect_path / x), reverse=True)
        idx = 0
        print(f"Using the latest training run: {train_folders[idx]}")
